...
| H-06 | 供电、热状态、负载和竞争进程的前置门禁可减少不合格硬件观察 | E1→E2 | C020：真实 preflight 识别 normalized load `0.363>0.25`、`mediaanalysisd 58.1%>25%` 并在 benchmark 前拒绝；42 tests 验证 fail-closed 治理 | 只证明能识别当前干扰，尚未证明有效 holdout 比例会提高；门禁后仍可能有调度/设备噪声 | 环境 PASS 后建立全新 fit/holdout cohort，并继续逐 Run 3% gate | PARTIAL_DETECTION_PROVEN_OUTCOME_PENDING |
