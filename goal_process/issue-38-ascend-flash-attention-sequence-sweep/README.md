...
- independent validation: 21 of 36 declared Run Bundles at the last successful
  read-only inventory;
...
The UniVPN-backed SSH path later accepted TCP/22 but closed before SSH key
exchange. The evidence therefore stops rather than changing Hardware Cohort,
adding experiments, or treating the partial corpus as qualified.
...
With a partial corpus this publisher emits a self-verifying structured unknown;
...
- `evidence/qualification-unknown.json`: bounded stopping decision, confirmed
  inventory, selected main medians, representative unknown queries, and exact
  replay commands.
