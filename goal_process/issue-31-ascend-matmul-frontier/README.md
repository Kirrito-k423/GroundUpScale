...
本目录保存 ticket #31 的可回放实现、真实 Ascend 910B2 测量证据，以及由这些证据资格化得到的最小 Capability Surface。权威结果是 `evidence/runs/issue31-operator-frontier-v3`；其 manifest 固定了全部 source Run Bundle 的相对路径与 SHA-256，因而候选、合同、cohort 或历史文件发生变化时，验证会失败而不是复用旧 Surface。资格门槛来自显式、版本化、cohort-scoped 的 `specs/policies/ascend-910b2-matmul-frontier-qualification-v1.yaml`，不存在隐式全局阈值。
...
- allowed candidate coverage：两候选属于两个 family，均通过 correctness、环境、计时、重复性与 C2 policy；每个候选都先完成独立 holdout，再做 best-of-correct
- best-of-correct：按独立 holdout 会话中位数，`torch.matmul` 在两个 Shape 上均胜出
- 独立 holdout：每个候选、每个 Anchor 3 个独立进程会话；`torch.matmul` 在 256 上为 16327.5、17190.5、16222.0 ns，在 512 上为 16224.5、16437.5、16331.5 ns；`torch.matmul.k-split-2` 在 256 上为 97033.5、95104.5、100448.0 ns，在 512 上为 91925.5、97415.0、99268.0 ns
...
| holdout v4 | 12 | 12 | 0 | 两个 eligible candidate 全部完成独立 holdout |
...
uv run groundupscale verify-run \
  goal_process/issue-31-ascend-matmul-frontier/evidence/runs/issue31-operator-frontier-v3
...
/home/t00906153/GroundUpScale-issue31-20260811/goal_process/issue-31-ascend-matmul-frontier/collect_frontier_evidence.sh holdout-v4 torch.matmul.k-split-2
